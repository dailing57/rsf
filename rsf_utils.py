...
'''
根据点云和包围盒计算权重。它使用sigmoid函数来计算每个点对每个包围盒的权重。
在计算完所有点的权重后,根据crop_threshold阈值将权重小于它的点从点云中去除。
最后返回新的点云、权重和原始权重。
如果normalize_weights=True,则在返回之前将每个点的权重归一化到[0,1]之间。
如果normals不为None,则函数还会返回新点云的法向量。
'''
